# All 78 Agents — Test Inputs Reference

Fictional brand: **GreenPulse** — sustainable fitness-tech company selling smart workout gear made from recycled ocean plastics.

**Base URL:** `http://localhost:8002/api/v1/agents/core`
**Generic endpoint:** `POST /{agent_id}/generate`

Optional (recommended) shared fields accepted by all v4 agents:
- `brief_primary_goal`, `brief_product_or_service`, `brief_offer`, `brief_call_to_action`, `brief_target_persona`, `brief_funnel_stage`
- `brief_key_points` (string[]), `brief_proof_points` (string[]), `brief_constraints` (string[])

Every agent below has its own unique, self-contained JSON payload. No shared references.

---

## 1. brand_identity

**Dedicated form** — uses `BrandIdentityForm`, not the generic agent form.
**Endpoint:** `POST /v4/brand_identity/generate`

```json
{
  "agent_id": "brand_identity",
  "workspace_id": "test-ws-001",
  "business_name": "GreenPulse",
  "business_description": "GreenPulse makes smart fitness wearables and workout gear crafted from recycled ocean plastics. We combine cutting-edge health tracking with sustainable manufacturing to help eco-conscious athletes perform their best while protecting the planet.",
...
```json
{
  ...
  "competitor_names": ["Fitbit", "Garmin", "Whoop", "Oura"],
  "target_audience": "Eco-conscious fitness enthusiasts aged 25-40"
}
```
...
```json
{
  ...
  "workspace_id": "test-ws-001",
  "product_description": "GreenPulse makes smart fitness wearables and workout gear crafted from recycled ocean plastics. We combine cutting-edge health tracking with sustainable manufacturing to help eco-conscious athletes perform their best while protecting the planet.",
  "brand_name": "GreenPulse",
  "industry": "Sustainable Fitness Technology",
...
```json
{
  ...
  "brand_description": "GreenPulse makes smart fitness wearables and workout gear crafted from recycled ocean plastics. We combine cutting-edge health tracking with sustainable manufacturing to help eco-conscious athletes perform their best while protecting the planet."
}
```
...
```json
{
  ...
  "workspace_id": "test-ws-001",
  "design_image_url": "https://greenpulse.example.com/images/greenpulse-logo.png",
...
## 20. instagram_carousel

```json
{
  "agent_id": "instagram_carousel",
  "workspace_id": "test-ws-001",
...
## 39. caption_generator

```json
{
  "agent_id": "caption_generator",
  "workspace_id": "test-ws-001",
...
## 41. video_trend_analyzer

```json
{
  "agent_id": "video_trend_analyzer",
  "workspace_id": "test-ws-001",
...
## 54. google_search_ads

```json
{
  "agent_id": "google_search_ads",
  "workspace_id": "test-ws-001",
  "product_name": "GreenPulse Pro Smart Band",
...
## 55. google_display_ads

```json
{
  "agent_id": "google_display_ads",
  "workspace_id": "test-ws-001",
  "product_name": "GreenPulse Pro Smart Band",
...
## 56. linkedin_lead_gen

```json
{
  "agent_id": "linkedin_lead_gen",
  "workspace_id": "test-ws-001",
...
```json
{
  "agent_id": "ab_test_setup",
  "workspace_id": "test-ws-001",
  "topic": "Test pricing display on GreenPulse Pro landing page: showing \"$199 early-bird\" vs \"$249 $199 (save $50)\" vs \"$199/mo x 4 payments\".",
  "test_type": "pricing",
  "target_audience": "Eco-conscious fitness enthusiasts aged 25-40",
...
## 73. marketing_psychology

```json
{
  "agent_id": "marketing_psychology",
  "workspace_id": "test-ws-001",
...
## 75. competitor_alternatives

```json
{
  "agent_id": "competitor_alternatives",
  "workspace_id": "test-ws-001",
...
## Quick curl test (any agent)

```bash
curl -X POST http://localhost:8002/api/v1/agents/core/v4/{agent_id}/generate \
  -H "Content-Type: application/json" \
  -d '{ PASTE JSON FROM ABOVE }'
```
